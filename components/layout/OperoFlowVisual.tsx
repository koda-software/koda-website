...
const flowStepClass = "relative grid content-start gap-4 rounded-[calc(var(--radius-panel)-10px)] border border-white/[0.09] bg-[#181b23]/80 p-[clamp(1.1rem,2.2vw,1.55rem)] shadow-[inset_0_1px_0_rgba(255,255,255,0.055),0_20px_55px_rgba(0,0,0,0.14)]";
const stepNumberClass = "block select-none font-sans text-[clamp(3.4rem,5.4vw,4.9rem)] font-semibold leading-[0.78] text-white/16";
const stepTitleClass = "m-0 text-[clamp(1.28rem,1.75vw,1.55rem)] font-medium leading-[1.1]";
const stepDescriptionClass = "mt-2 max-w-[28rem] text-[0.98rem] font-light leading-[1.58] text-white/64";
const chipBaseClass = "rounded-full border border-white/[0.1] px-4 py-2.5 font-sans text-[0.88rem] font-normal leading-none";
const mappingUnitBaseClass = "relative min-h-[6.4rem] overflow-hidden rounded-[calc(var(--radius-card)+2px)] border border-white/[0.1] bg-[#11141b]/72 px-4 py-4 font-sans text-[0.92rem] font-light leading-[1.4] text-white/74 after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-[linear-gradient(90deg,transparent,rgba(158,227,125,0.45),transparent)] [&>i]:mb-3 [&>i]:flex [&>i]:h-7 [&>i]:w-7 [&>i]:items-center [&>i]:justify-center [&>i]:rounded-full [&>i]:border [&>i]:border-[rgba(158,227,125,0.2)] [&>i]:bg-[rgba(158,227,125,0.1)] [&>i>svg]:h-4 [&>i>svg]:w-4 [&>i>svg]:text-[var(--color-signal)]";
const outcomeItemClass = "flex items-center gap-3 text-[0.96rem] font-light leading-[1.5] text-white/82";
...
export function OperoFlowVisual({ content }: OperoFlowVisualProps) {
  return (
    ...
    >
      <article className={flowStepClass}>
