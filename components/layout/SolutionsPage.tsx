...
const titleClass = "m-0 max-w-[900px] text-[3.1rem] leading-[1.05] text-[var(--color-ink)] max-[809px]:text-[2.15rem]";
...
function DetailList({ items }: { items: string[] }) {
  return (
    <ul className="mt-4 grid list-none gap-3 p-0 text-[0.98rem] font-light leading-[1.6] text-[var(--color-muted)]">
      {items.map((item) => (
        <li className="border-t border-[rgba(2,2,13,0.08)] pt-3 first:border-t-0 first:pt-0" key={item}>
          {item}
        </li>
      ))}
    </ul>
...
function IndustrySection({ industry, index }: { industry: SolutionIndustry; index: number }) {
  ...
  return (
    <section className={`border-t border-[rgba(2,2,13,0.12)] ${index % 2 === 1 ? "bg-[var(--color-paper-soft)]" : "bg-[var(--color-paper)]"}`}>
      <div className={`${sectionClass} grid grid-cols-[minmax(0,0.45fr)_minmax(0,1fr)] gap-16 max-[980px]:grid-cols-1 max-[980px]:gap-8`}>
        <div>
          <div className="mb-6 flex items-center gap-4">
            <span className="flex h-12 w-12 items-center justify-center rounded-[var(--radius-button)] border border-[rgba(2,2,13,0.12)] bg-white text-[var(--color-blue)] shadow-[0_10px_28px_rgba(2,2,13,0.045)]">
              <IndustryIcon className="h-5 w-5" strokeWidth={1.7} aria-hidden="true" />
            </span>
            <span className="text-[0.88rem] font-semibold text-[var(--color-blue)]">{String(index + 1).padStart(2, "0")}</span>
          </div>
          <h3 className="m-0 max-w-[34rem] text-[2.35rem] font-medium leading-[1.08] text-[var(--color-ink)] max-[809px]:text-[1.8rem]">
            {industry.title}
          </h3>
        </div>
        <div className="grid gap-10">
          ...
          <div className="grid grid-cols-2 gap-10 max-[809px]:grid-cols-1">
            <div>
              <h4 className="m-0 text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-[var(--color-muted)]">{industry.useCasesLabel}</h4>
              <DetailList items={industry.useCases} />
            </div>
            <div>
              <h4 className="m-0 text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-[var(--color-muted)]">{industry.helpfulFeaturesLabel}</h4>
              <DetailList items={industry.helpfulFeatures} />
            </div>
          </div>

          <div className="border-t border-[rgba(2,2,13,0.12)] pt-6">
            <h4 className="m-0 text-[0.78rem] font-semibold uppercase tracking-[0.08em] text-[var(--color-muted)]">{industry.supportsLabel}</h4>
            <ul className="mt-4 grid list-none grid-cols-2 gap-x-10 gap-y-3 p-0 text-[0.96rem] leading-[1.55] text-[var(--color-ink-soft)] max-[809px]:grid-cols-1">
              {industry.supports.map((support) => (
                <li className="border-b border-[rgba(2,2,13,0.08)] pb-2" key={support}>
                  {support}
                </li>
              ))}
            </ul>
  ...
  return (
    <>
      <section className="bg-[#02020d] [background-image:var(--gradient-hero)] px-[var(--page-gutter)] pb-[5.5rem] pt-[10rem] text-white max-[809px]:pt-28">
        <div className="mx-auto grid w-[min(100%,var(--shell-width))] grid-cols-[minmax(0,1fr)_minmax(18rem,0.52fr)] items-end gap-16 max-[980px]:grid-cols-1">
          ...
          <div className="border-y border-white/[0.12] py-8">
            <p className="m-0 text-[1.08rem] font-light leading-[1.65] text-white/68">{content.hero.supportLine}</p>
          </div>
        </div>
      </section>
      ...
      {content.industries.items.map((industry, index) => (
        <IndustrySection industry={industry} index={index} key={industry.title} />
      ))}
        ...
        <div className="mx-auto w-[min(100%,var(--shell-width))] rounded-[var(--radius-panel)] [background:var(--gradient-technical)] p-[3.5rem] text-white max-[809px]:p-6">
          <div className="grid grid-cols-[minmax(0,0.72fr)_minmax(22rem,0.86fr)] gap-12 max-[980px]:grid-cols-1">
            <div>
              ...
              <h2 className="m-0 max-w-[760px] text-[3rem] leading-[1.06] max-[809px]:text-[2.05rem]">{content.pattern.title}</h2>
              <p className="mt-6 max-w-[680px] text-[1.04rem] font-light leading-[1.7] text-white/68">{content.pattern.description}</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[34rem] border-collapse text-left">
                <thead className="text-[0.72rem] uppercase tracking-[0.08em] text-white/42">
                  <tr>
                    <th className="w-[45%] border-b border-white/[0.12] pb-4 pr-6 font-semibold">{content.pattern.problemLabel}</th>
                    <th className="border-b border-white/[0.12] pb-4 font-semibold">{content.pattern.supportLabel}</th>
                  </tr>
                </thead>
                <tbody>
                  {content.pattern.rows.map((row) => (
                    <tr className="border-b border-white/[0.08] last:border-b-0" key={row.problem}>
                      <td className="py-4 pr-6 align-top text-[0.96rem] font-medium leading-[1.5] text-white/82">{row.problem}</td>
                      <td className="py-4 align-top text-[0.96rem] font-light leading-[1.6] text-white/62">{row.support}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <section className={`${sectionClass} grid grid-cols-[minmax(0,0.58fr)_minmax(0,1fr)] gap-16 max-[980px]:grid-cols-1`}>
        <div>
          <p className={eyebrowClass}>{content.fit.eyebrow}</p>
          <h2 className={titleClass}>{content.fit.title}</h2>
          <p className={`${bodyClass} mt-5 max-w-[680px]`}>{content.fit.description}</p>
        </div>
        <div className="grid grid-cols-2 gap-8 max-[809px]:grid-cols-1">
          <div className="border-y border-[rgba(2,2,13,0.12)] py-6">
            <h3 className="m-0 text-[1.35rem] font-medium text-[var(--color-ink)]">{content.fit.goodFitTitle}</h3>
          ...
          <div className="border-y border-[rgba(2,2,13,0.12)] py-6">
            <h3 className="m-0 text-[1.35rem] font-medium text-[var(--color-ink)]">{content.fit.notBestFitTitle}</h3>
            <ul className="mt-5 grid list-none gap-3 p-0 text-[0.98rem] font-light leading-[1.6] text-[var(--color-muted)]">
      ...
      <section className={`${sectionClass} pt-0`}>
        <div className="border-y border-[rgba(2,2,13,0.12)] py-[4rem] text-center">
          ...
          <h2 className="mx-auto m-0 max-w-[860px] text-[3.15rem] leading-[1.04] text-[var(--color-ink)] max-[809px]:text-[2.1rem]">
